...
class Hub : public cSimpleModule
{
    ...
    protected:
        virtual void initialize();                          // This initializes
        virtual void handleMessage(cMessage *msg);          // This does the hard work
        virtual void forwardMessage(cMessage *msg);         // This forwards a message to a specific person
        virtual void forwardMessageToAll(cMessage *msg);    // This forwards a message to everyone
        bool isLost(cMessage *msg);                         // This checks if a message was lost
};
...
void Hub::handleMessage(cMessage *msg)
{
    ...
    if(msg == event)
    {
        ...
        if(isLost(tictocMsg))
        {
            EV << "I don't know who lost a message, telling everyone to check if it was theirs\n";
            forwardMessageToAll(tictocMsg);
        }
        else
        {
            EV << "Forwarding this message to whom it was for\n";
     ...
     {
          // Pass to all
          EV << "I'll forward this in "<< delay << " seconds\n";
          tictocMsg = msg;
